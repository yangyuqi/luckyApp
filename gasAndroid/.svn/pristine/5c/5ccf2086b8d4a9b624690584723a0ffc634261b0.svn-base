...
//		return framingRect;
		  ...
		  int screenWidth = ViewfinderView.screenWidth;
		  int screenHeight = ViewfinderView.screenHeight;
		  int iconHeight = 0;
		  if(ViewfinderView.iconTipDrawable != null) {
			  iconHeight = ViewfinderView.iconTipDrawable.getIntrinsicHeight();
		  }

		  int scanHeight = screenHeight * 56 /100 - DensityUtil.dip2px(context, 68) - DensityUtil.dip2px(context, 20) * 2 
				           - ViewfinderView.ICON_PADDING_TOP * ((int)ViewfinderView.density) - iconHeight;
		  int width = scanHeight;
		  int height = scanHeight;
//		  int topOffset = DensityUtil.dip2px(context, 20 + 48);
		int topOffset = (screenHeight - height) /2 - DensityUtil.dip2px(context, 40);
		int leftOffset = (screenWidth - width) /2;
//		  int screenWidth = screenResolution.x;
//		  int width = DensityUtil.dip2px(context, 190);
//		  int height = width;
//		  int topOffset = DensityUtil.dip2px(context, 68);
//		  int leftOffset = (screenWidth - width) /2;
		  return framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
	}
...
//	}
	...
	public void setZoom() {
		if (camera != null && camera.getParameters().isZoomSupported() && camera.getParameters().isSmoothZoomSupported()) {
			Camera.Parameters parameters = camera.getParameters();
